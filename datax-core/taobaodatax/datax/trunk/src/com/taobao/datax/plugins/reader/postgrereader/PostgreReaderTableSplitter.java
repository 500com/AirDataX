...
public class PostgreReaderTableSplitter extends Splitter {
	...
	private Logger logger = Logger.getLogger(PostgreReaderTableSplitter.class);
	
	public PostgreReaderTableSplitter(PluginParam iParam){
		param = iParam;
	}
	
	@Override
	public int init() {
		logger.info("PostgreReaderTableSplitter initialize begins .");

		tables = SplitUtils.splitTables(param.getValue(ParamKey.tables));
        logger.info("split table size:" + tables.size());
