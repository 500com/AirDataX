...
public class DBSource {
	...
	public static synchronized boolean register(String key, Properties p) {
        ...
        p.setProperty("maxWait","180000");
        p.setProperty("testOnBorrow","true");
        logger.info(String.format(
                "Key [%s] register database pool", key));
		if (!sourceInfoMap.containsKey(key)) {
			...
			try {
				...
				logger.error(String.format(
						"Key [%s] register database pool failed .", key),e);
			...
			logger.info(String.format("Key [%s] already in database pool .",
                    key));
		}
	...
	public static DataSource getDataSource(Class<? extends Pluginable> clazz,
			String ip, String port, String dbname,String username,String password) {
		return getDataSource(genKey(clazz, ip, port, dbname,username,password));
	}
	...
	public static Connection getConnection(Class<? extends Pluginable> clazz,
			String ip, String port, String dbname,String username,String password) {
		return getConnection(genKey(clazz, ip, port, dbname,username,password));
	}
	...
	public static String genKey(Class<? extends Pluginable> clazz, String ip,
		...
		String str = clazz.getCanonicalName() + "_:_" + ip + "_:_" + port + "_:_"
				+ dbname + "_:_"  + username + "_:_" + password ;
        logger.info(String.format("%s_:_%s_:_%s_:_%s_:_%s_:_******",clazz.getCanonicalName(),ip,port,dbname,username));
		return md5(str);
	}
