...
public class Reporter {
	...
	public String parseJobConf(JobConf jobConf) {
        ...
        log.append("JOB_ID=").append(jobConf.getJobId());
		log.append("&CONF_ID=").append(jobConf.getId());
	...
	public void report(JobConf jobConf) {
		...
		try {
			...
			connection.setReadTimeout(1000 * 5);// 5 seconds read time out
			try {
				...
				if ("0".equals(rsStatus)) {
                    LOG.info("Report DataX statistic("+jobConf.getJobId()+") successfully .");
				}else {
                    LOG.info("Report DataX statistic("+jobConf.getJobId()+") failed ." + rsStatus);
                }

			} catch (Exception ex) {
				LOG.info("Report DataX statistic("+jobConf.getJobId()+") failed",ex);
			} finally {
				connection.disconnect();
			}
		}  catch (Exception e) {
			LOG.info("Report DataX statistic("+jobConf.getJobId()+") failed",e);
		}
	}
}
